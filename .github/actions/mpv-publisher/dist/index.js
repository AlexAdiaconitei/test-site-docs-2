...
function assertManifest(value) {
    if (value?.schemaVersion !== 1)
        throw new Error("Unsupported docs-template.json schemaVersion");
    if (!value.adapterKey || !value.content?.docsTarget || !value.build?.outputDir)
        throw new Error("Invalid docs-template.json");
    return value;
}
function assertPublication(value) {
    if (value?.schemaVersion !== 1)
        throw new Error("Unsupported publication.json schemaVersion");
    if (!value.template?.repository || !value.template?.ref || !value.content?.docsSourcePath)
        throw new Error("Invalid publication config");
    return value;
}
async function main() {
    ...
    if (!adapter)
        throw new Error(`Unsupported adapter: ${publication.template.adapterKey}`);
    ...
    if (manifest.adapterKey !== adapter.key)
        throw new Error(`Template adapter ${manifest.adapterKey} does not match publication adapter ${adapter.key}`);
